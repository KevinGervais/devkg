...
export async function updateOneEmail({ currentUser, body, mongodb, send, refreshCurrentUser }: SocketParams<"users", "updateOne">): Promise<void> {
  ...
  const foundEmailUser = await mongodb.getOne("users", { email })
  if (foundEmailUser) {
    throwError("userAlreadyExists")
  }
  await mongodb.updateOne("users", { _id: currentUser._id }, { $set: { emailToken } })
  send(undefined)
